...
const menu_opcoes =  document.querySelector("#menu_opcoes") as HTMLSelectElement
const todas_tarefas =  document.querySelector("#todas_tarefas") as HTMLOptionElement
const tarefas_incompletas =  document.querySelector("#tarefas_incompletas") as HTMLOptionElement
const tarefas_completas =  document.querySelector("#tarefas_completas") as HTMLOptionElement
...
const adicionarElementoTarefa = (tarefa: { titulo: string; hora: string; data: string; completa: boolean;}) => {
  ...
  const agendarNotificacao = (titulo: string, descricao: string, diferenca: number, tarefaCompleta?: boolean) => {
  ...
  bt_pesquisar.addEventListener("click",()=>{
   ...
   menu_opcoes.addEventListener("change", () => {
    ...
    const tarefas_filtradas = array_tarefas.filter(tarefa => {
        if (todas_tarefas.selected) return true
        if (tarefas_completas.selected) return tarefa.completa
        if (tarefas_incompletas.selected) return !tarefa.completa
    })
